...
void led_toggle(struct k_timer *dummy)
{
    static int state = 0;
    state = 1 - state;
    gpio_pin_write(device_get_binding(LED1_GPIO_CONTROLLER), LED1_GPIO_PIN, state);
}
K_TIMER_DEFINE(led_timer, led_toggle, NULL);

void led_init(void)
{
    k_timer_start(&led_timer, K_SECONDS(1), K_SECONDS(1));
    gpio_pin_configure(device_get_binding(LED1_GPIO_CONTROLLER), LED1_GPIO_PIN, GPIO_DIR_OUT);
    gpio_pin_write(device_get_binding(LED1_GPIO_CONTROLLER), LED1_GPIO_PIN, 0);
}
/*
...
do {                                                                    \
    ...
    {                                                                   \
        double count = (double)ii/60;                                   \
        mbedtls_printf( "%6.2f " TYPE "/s", count );                    \
...
char *options[] = {"rsa", "sha1", "sha256", "sha512"};
int main( int argc, char *argv[] )
{
    int i;
    unsigned char tmp[200];
    char title[TITLE_LEN] = {0x00, };
    todo_list todo;
